...
namespace xNode
{
    ...
    public static class NodeDataCache
    {
        ...
        public static List<FieldInfo> GetNodeFields(System.Type nodeType)
        {
            ...
            var fieldInfo = new List<FieldInfo>(nodeType.GetFields(bindingFlags));
        ...
        private static void CachePorts(System.Type nodeType)
        {
            ...
            for (var i = 0; i < fieldInfo.Count; i++)
            {
                ...
                var attributes = fieldInfo[i].GetCustomAttributes(true);

                var inputAttribute = (Node.InputAttribute) attributes.FirstOrDefault(x => x is Node.InputAttribute);
                var outputAttribute = (Node.OutputAttribute) attributes.FirstOrDefault(x => x is Node.OutputAttribute);
